...
#define TEST_BASE_MQ_MESSAGE_HANDLE         (MQ_MESSAGE_HANDLE)0x10000
...
static void add_messages(MESSAGE_QUEUE_HANDLE mq, size_t number_of_messages, time_t current_time)
{
	...
	for (i = 0; i < number_of_messages; i++)
	{
		...
		int result = message_queue_add(mq, (MQ_MESSAGE_HANDLE)((unsigned int)TEST_BASE_MQ_MESSAGE_HANDLE + i), TEST_on_message_processing_completed_callback, TEST_USER_CONTEXT);
...
TEST_FUNCTION(add_NULL_mq_handle)
{
	...
	// act
	int result = message_queue_add(NULL, TEST_BASE_MQ_MESSAGE_HANDLE, TEST_on_message_processing_completed_callback, TEST_USER_CONTEXT);
...
TEST_FUNCTION(add_success)
{
	...
	// act
	int result = message_queue_add(mq, TEST_BASE_MQ_MESSAGE_HANDLE, TEST_on_message_processing_completed_callback, TEST_USER_CONTEXT);
...
TEST_FUNCTION(do_work_NO_EXPIRATION_success)
{
	...
	// assert
	ASSERT_ARE_EQUAL(char_ptr, umock_c_get_expected_calls(), umock_c_get_actual_calls());
	ASSERT_ARE_EQUAL(void_ptr, (void_ptr)TEST_BASE_MQ_MESSAGE_HANDLE, (void_ptr)TEST_on_process_message_callback_message);
...
TEST_FUNCTION(on_message_processing_completed_callback_success)
{
	...
	add_messages(mq, 1, TEST_current_time);
	crank_message_queue(mq, TEST_current_time, 1, 0, NULL);
	ASSERT_ARE_EQUAL(void_ptr, (void*)TEST_BASE_MQ_MESSAGE_HANDLE, (void*)TEST_on_process_message_callback_message);
	ASSERT_IS_NOT_NULL(TEST_on_process_message_callback_on_process_message_completed_callback);
