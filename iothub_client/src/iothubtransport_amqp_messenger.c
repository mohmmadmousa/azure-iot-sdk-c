...
typedef struct MESSENGER_SEND_EVENT_TASK_TAG
{
	...
	ON_AMQP_MESSENGER_SEND_COMPLETE on_send_complete_callback;
	void* context;
	time_t send_time;
...
static void destroy_event_sender(AMQP_MESSENGER_INSTANCE* instance)
{
...
static int create_event_sender(AMQP_MESSENGER_INSTANCE* instance)
{
	int result;
	...
	{
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_036: [If `event_send_address` fails to be created, amqp_messenger_do_work() shall fail and return]
		result = __FAILURE__;
...
static AMQP_VALUE on_message_received_internal_callback(const void* context, MESSAGE_HANDLE message)
{
	...
	else
	{
		...
		else
		{
			...
			AMQP_MESSENGER_DISPOSITION_RESULT disposition_result = instance->on_message_received_callback(iothub_message, message_disposition_info, instance->on_message_received_context);

			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_188: [The memory allocated for the AMQP_MESSENGER_MESSAGE_DISPOSITION_INFO instance shall be released]
...
static int create_message_receiver(AMQP_MESSENGER_INSTANCE* instance)
{
	...
	{
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_071: [If `message_receive_address` fails to be created, amqp_messenger_do_work() shall fail and return]
		result = __FAILURE__;
...
static int move_events_to_wait_to_send_list(AMQP_MESSENGER_INSTANCE* instance)
{
	int result;
	LIST_ITEM_HANDLE list_item;
...
static void internal_on_send_complete_callback(void* context, MESSAGE_SEND_RESULT send_result)
{ 
	if (context != NULL)
	{
		...
		if (task->messenger->message_sender_current_state != MESSAGE_SENDER_STATE_ERROR)
		{
			...
			{
				AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT messenger_send_result;
				...
				{
					messenger_send_result = AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_OK;
				}
				...
				{
					messenger_send_result = AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING;
				}

				task->on_send_complete_callback(task->message, messenger_send_result, (void*)task->context);
			}
			else
			{
				LogInfo("messenger on_send_complete_callback invoked for timed out event %p; not firing upper layer callback.", task->message);
			}
...
static MESSENGER_SEND_EVENT_TASK* get_next_event_to_send(AMQP_MESSENGER_INSTANCE* instance)
{
...
static int send_pending_events(AMQP_MESSENGER_INSTANCE* instance)
{
	...
	{
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_153: [amqp_messenger_do_work() shall move each event to be sent from `instance->wait_to_send_list` to `instance->in_progress_list`] 
		if (move_event_to_in_progress_list(task) != RESULT_OK)
		{
			result = __FAILURE__;
			task->on_send_complete_callback(task->message, AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING, (void*)task->context);
			break;
		}
		else
		{
			...
			if ((uamqp_result = message_create_from_iothub_message(task->message->messageHandle, &amqp_message)) != RESULT_OK)
			{
				...
				// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_155: [If message_create_from_iothub_message() fails, `task->on_send_complete_callback` shall be invoked with result EVENT_SEND_COMPLETE_RESULT_ERROR_CANNOT_PARSE]  
				task->on_send_complete_callback(task->message, AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_ERROR_CANNOT_PARSE, (void*)task->context);
				...
				// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_156: [If message_create_from_iothub_message() fails, amqp_messenger_do_work() shall skip to the next event to be sent]  
			}
			else
			{
				// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_157: [The MESSAGE_HANDLE shall be submitted for sending using messagesender_send(), passing `internal_on_send_complete_callback`]  
				uamqp_result = messagesender_send(instance->message_sender, amqp_message, internal_on_send_complete_callback, task);
				...
				if (uamqp_result != RESULT_OK)
				{
					...
					// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_158: [If messagesender_send() fails, `task->on_send_complete_callback` shall be invoked with result EVENT_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING]
					task->on_send_complete_callback(task->message, AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING, (void*)task->context);
...
static int process_event_send_timeouts(AMQP_MESSENGER_INSTANCE* instance)
{
	int result = RESULT_OK;
			...
			if (task->is_timed_out == false)
			{
				...
				if (is_timeout_reached(task->send_time, instance->event_send_timeout_secs, &is_timed_out) == RESULT_OK)
				{
					if (is_timed_out)
					{
						...
						if (task->on_send_complete_callback != NULL)
						{
							task->on_send_complete_callback(task->message, AMQP_MESSENGER_EVENT_SEND_COMPLETE_RESULT_ERROR_TIMEOUT, task->context);
						}
					}
				}
				else
...
static void remove_timed_out_events(AMQP_MESSENGER_INSTANCE* instance)
{
	LIST_ITEM_HANDLE list_item = singlylinkedlist_get_head_item(instance->in_progress_list);
...
int amqp_messenger_send_async(AMQP_MESSENGER_HANDLE messenger_handle, IOTHUB_MESSAGE_LIST* message, ON_AMQP_MESSENGER_SEND_COMPLETE on_messenger_event_send_complete_callback, void* context)
{
	int result;
	...
	// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_135: [If `message` is NULL, amqp_messenger_send_async() shall fail and return a non-zero value]  
	else if (message == NULL)
	{
		LogError("Failed sending event (message is NULL)");
		result = __FAILURE__;
	}
	// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_136: [If `on_send_complete_callback` is NULL, amqp_messenger_send_async() shall fail and return a non-zero value] 
	else if (on_messenger_event_send_complete_callback == NULL)
	{
		LogError("Failed sending event (on_send_complete_callback is NULL)");
		result = __FAILURE__;
	}
	else
	{
		...
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_137: [amqp_messenger_send_async() shall allocate memory for a MESSENGER_SEND_EVENT_TASK structure (aka `task`)]  
		if ((task = (MESSENGER_SEND_EVENT_TASK*)malloc(sizeof(MESSENGER_SEND_EVENT_TASK))) == NULL)
		{
			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_138: [If malloc() fails, amqp_messenger_send_async() shall fail and return a non-zero value]
		...
		else if (singlylinkedlist_add(instance->waiting_to_send, task) == NULL)
		{
			...
			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_139: [If singlylinkedlist_add() fails, amqp_messenger_send_async() shall fail and return a non-zero value]
			result = __FAILURE__;

			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_142: [If any failure occurs, amqp_messenger_send_async() shall free any memory it has allocated]
			free(task);
		}
		else
		{
			...
			task->on_send_complete_callback = on_messenger_event_send_complete_callback;
			task->context = context;
			task->send_time = INDEFINITE_TIME;
			task->messenger = instance;
			task->is_timed_out = false;
			
			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_143: [If no failures occur, amqp_messenger_send_async() shall return zero]  
			result = RESULT_OK;
		}
	}
...
int amqp_messenger_get_send_status(AMQP_MESSENGER_HANDLE messenger_handle, AMQP_MESSENGER_SEND_STATUS* send_status)
{
	...
	if (messenger_handle == NULL)
	{
		LogError("amqp_messenger_get_send_status failed (messenger_handle is NULL)");
		result = __FAILURE__;
	}
	// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_145: [If `send_status` is NULL, amqp_messenger_get_send_status() shall fail and return a non-zero value] 
	else if (send_status == NULL)
	{
		LogError("amqp_messenger_get_send_status failed (send_status is NULL)");
		result = __FAILURE__;
	}
	else
	{
		...
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_147: [If `instance->in_progress_list` and `instance->wait_to_send_list` are empty, send_status shall be set to AMQP_MESSENGER_SEND_STATUS_IDLE] 
		if (wts_list_head == NULL && ip_list_head == NULL)
		{
			*send_status = AMQP_MESSENGER_SEND_STATUS_IDLE;
		}
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_148: [Otherwise, send_status shall be set to AMQP_MESSENGER_SEND_STATUS_BUSY] 
		else
		{
			*send_status = AMQP_MESSENGER_SEND_STATUS_BUSY;
		}

		// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_149: [If no failures occur, amqp_messenger_get_send_status() shall return 0]
		result = RESULT_OK;
	}
...
static void on_process_message_callback()
{
...
void amqp_messenger_do_work(AMQP_MESSENGER_HANDLE messenger_handle)
{
	...
	else
	{
		...
		else if (instance->state == AMQP_MESSENGER_STATE_STARTED)
		{
			...
			{
				LogError("amqp_messenger_do_work warning (failed creating the message receiver [%s])", STRING_c_str(instance->device_id));
			}
			...
			else if (send_pending_events(instance) != RESULT_OK && instance->event_send_retry_limit > 0)
			{
				...
				{
					LogError("amqp_messenger_do_work failed (failed sending events; reached max number of consecutive attempts)");
					update_messenger_state(instance, AMQP_MESSENGER_STATE_ERROR);
				}
			}
			else
...
void amqp_messenger_destroy(AMQP_MESSENGER_HANDLE messenger_handle)
{
	...
	else
	{
		...
		{
			STRING_delete(instance->devices_path_format);
		}
		if (instance->send_link_target_suffix != NULL)
		{
			STRING_delete(instance->send_link_target_suffix);
		}
		if (instance->receive_link_source_suffix != NULL)
		{
			STRING_delete(instance->receive_link_source_suffix);
		}
...
OPTIONHANDLER_HANDLE amqp_messenger_retrieve_options(AMQP_MESSENGER_HANDLE messenger_handle)
{
	...
	else
	{
		...
		OPTIONHANDLER_HANDLE result = OptionHandler_Create(amqp_messenger_clone_option, amqp_messenger_destroy_option, (pfSetOption)amqp_messenger_set_option);
		...
		else
		{
			...
			else if (OptionHandler_AddOption(result, MESSENGER_OPTION_EVENT_SEND_TIMEOUT_SECS, (void*)mq_options) != OPTIONHANDLER_OK)
			{
